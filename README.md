...
常驻系统托盘的小工具，支持 Windows 和 Ubuntu，实时监控 DeepSeek API 账户余额、消费金额、Token 用量和缓存命中率。
...
1. 从 [Release 页面](https://github.com/KyleCaiZJU/deepseek-monitor/releases) 下载最新安装包（推荐 `_x64-setup.exe`）
2. 双击安装，一路下一步即可
3. 安装完成后，Win 键搜索 **DeepSeek** 启动

### Linux (Ubuntu)

1. 从 [Release 页面](https://github.com/KyleCaiZJU/deepseek-monitor/releases) 下载 `.deb` 或 `.AppImage`
2. **deb 安装**：`sudo dpkg -i deepseek-monitor_*.deb`，然后从应用菜单搜索 **DeepSeek** 启动
3. **AppImage**：`chmod +x deepseek-monitor_*.AppImage && ./deepseek-monitor_*.AppImage`

> 依赖：需要 `libwebkit2gtk-4.1-dev` 和 `libayatana-appindicator3-dev`（Ubuntu 22.04+ 通常已预装 WebKitGTK，如缺少可 `sudo apt install libwebkit2gtk-4.1-dev libayatana-appindicator3-dev`）
...
- 所有数据存储在本地 SQLite 数据库中（Windows：`%APPDATA%\deepseek-monitor\deepseek-monitor.db`；Linux：`~/.local/share/deepseek-monitor/deepseek-monitor.db`），不上传任何第三方
...
v1.0.1 已修复：窗口定位现在正确处理 DPI 缩放，首次弹出位于屏幕右下角。如果仍有问题，请升级到最新版本。
...
- **Windows**：通过 Windows 设置 -> 应用 -> 找到 DeepSeek Monitor -> 卸载。数据库文件需手动删除：`%APPDATA%\deepseek-monitor\`
- **Linux**：deb 安装执行 `sudo dpkg -r deepseek-monitor`，AppImage 直接删除文件。数据库文件需手动删除：`~/.local/share/deepseek-monitor/`

### Q: Ubuntu 上托盘图标不显示

- 确保已安装 `libayatana-appindicator3-dev`：`sudo apt install libayatana-appindicator3-dev`
- 部分桌面环境（如 GNOME）可能需要安装 `gnome-shell-extension-appindicator` 扩展
